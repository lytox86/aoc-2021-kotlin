...
fun main() {
    ...
    fun solve(input: Program, digits: IntProgression): Long {
        return solve(VirtualMachine(input), digits, mutableSetOf(), 0) ?: throw Exception()
    }

    fun part1(input: List<String>): Long {
        return solve(Program.parse(input), 9 downTo 1)
    }

    fun part2(input: List<String>): Long {
        return solve(Program.parse(input), 1..9)
    }

    val input = readInput("Day24")
    println(part1(input)) // 99911993949684
    println(part2(input)) // 62911941716111
}
